...
### Residual connection : tackling gradient vanishing problem
...
''' 
Also the baseline MLP baseline translator with residual connections.
Unlike normalization in Linear Baseline, this MLP translator does not normalize the output
but normalizes during the forward pass; layer normalization is adopted as a default option.
'''
class MLPWithResidual(nn.Module):
    def __init__(
        ...
        ### This is somewhat hard-coded for now.
        ### If I write this code again, I would like to use enumeration so that
        ### we can prevent the user from making a mistake.
            ...
            if layer_idx == 0:
                ...
                self.layers.append(
                    nn.Sequential(
                        ...
                        nn.SiLU(),
                        # norm_layer(hidden_dim), -> They originally used normalization for the first layer but discarded it.
                    )
                )
